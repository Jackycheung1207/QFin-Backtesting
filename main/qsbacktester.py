...
def assign_position(df:pd.DataFrame, threshold:float,isMomentum:bool):
    """
...
def denoise(df:pd.DataFrame,denoise_logic:str, lookback_period:int):
    ...
    elif denoise_logic=='ln+z':
        df['metric'] = np.log(df['metric'])
        df['ma'] = df['metric'].rolling(lookback_period).mean()
        df['sd'] = df['metric'].rolling(lookback_period).std()
        df['signal'] = (df['metric'] - df['ma']) / df['sd']

    elif denoise_logic=='pct+z':
        df['metric'] =  df['metric'].pct_change()
        df['ma'] = df['metric'].rolling(lookback_period).mean()
        df['sd'] = df['metric'].rolling(lookback_period).std()
        df['signal'] = (df['metric'] - df['ma']) / df['sd']

    elif denoise_logic=='ln+pct+z':
        df['metric'] = np.log(df['metric'])
        df['metric'] = df['metric'].pct_change()
        df['ma'] = df['metric'].rolling(lookback_period).mean()
        df['sd'] = df['metric'].rolling(lookback_period).std()
...
def compute_expected_return_(df:pd.DataFrame,style:str='long&short'):
    if style == 'long&short':
        suffix = ''
...
def get_price_df(price_folder_path:str,coin:str,tf:str,exchange:str,instrument:str):
    price_file_name = f'{coin}_{tf}_{exchange}_{instrument}.csv'
...
def get_price_metric_df(price_folder_path:str,coin:str,tf:str,exchange:str,instrument:str):
    price_file_name = f'{coin}_{tf}_{exchange}_{instrument}.csv'
    logger.info(f"READ: {price_file_name}")
...
def get_metric_df(metric_folder_path:str,metrics_name:str,coin:str,tf:str):
    if tf != '1d': tf = '1h'
    metric_file_name = f'{metrics_name}_{coin}_{tf}.csv'
    logger.info(f"READ: {metric_file_name}")
    metric_df = pd.read_csv(
        os.path.join(metric_folder_path, metric_file_name),index_col=0
    ).reset_index()
    metric_df = metric_df[['timestamp', metrics_name]]
    metric_df = metric_df.rename(columns={metrics_name: 'metric'})
    return metric_df

def data_preprocessing(price_df:pd.DataFrame,metric_df:pd.DataFrame,
                       metrics_name:str,shift:int=2,START_DATE:str='2020-06-01',END_DATE:str='2024-03-31'):
    ...
    metric_df['metric'] = metric_df['metric'].shift(shift) #TODO: shift 2 bar / hours
...
def calculations(df:pd.DataFrame,threshold:float, isMomentum:bool,
    ...
    df = assign_position(df, threshold, isMomentum)
...
def compute_kpi(df:pd.DataFrame,tf_int:int):
    ...
    if position_counts_normalized.get('no position')!=1:
        try:
            kpi_dict['long_dominance_count'] = round(position_counts_normalized.get('long')/(position_counts_normalized.get('long')+position_counts_normalized.get('short')),4)
        except:
    ...
    return kpi_dict
def backtesting( coin:str,tf:str,metrics_name:str,
                ...
                isPlot:bool=False,train_size:float=0.7,shift=None,unit_tc:float=0.0006,is_save_ts:bool=False):
    ...
    price_df = get_price_df(price_folder_path,coin,tf,exchange,instrument)

    if metric_type=='price':
        metric_df = get_price_metric_df(price_folder_path,coin,tf,exchange,instrument)
    elif metric_type=='onchain':
        metric_df = get_metric_df(metric_folder_path,metrics_name,coin,tf)
    else:
        raise ValueError(f"metric_type: {metric_type} is not supported")

    df_clean = data_preprocessing(price_df,metric_df,metrics_name,shift=shift)
    ...
    df_denoise=calculations(df_denoise, threshold, isMomentum, unit_tc)
    df_denoise_train=calculations(df_denoise_train, threshold, isMomentum, unit_tc)
    df_denoise_test=calculations(df_denoise_test, threshold, isMomentum, unit_tc)
    ...
    full_set_dict = compute_kpi(df_denoise, tf_int)
    ...
    if isPlot:
        ...
        fig.add_trace(go.Scatter(x=df_denoise_train.index, y=df_denoise_train['strategy_ret'], name='Strategy Returns',
        ...
        fig.add_trace(go.Scatter(x=df_denoise_test.index, y=df_denoise_test['strategy_ret'], name='Strategy Returns',
                                 line=dict(color='blue')),
                      row=1, col=2)
        fig.add_trace(go.Scatter(x=df_denoise_test.index, y=df_denoise_test['benchmark_ret'], name='Benchmark Returns',
                                 line=dict(color='orange')),
                      row=1, col=2)
        ...
        fig.add_trace(
            ...
            row=3, col=1)
        fig.add_trace(
            go.Scatter(x=df_denoise_train.index, y=df_denoise_train['benchmark_drawdown'], name='Benchmark Drawdown',
                       line=dict(color='orange')),
            row=3, col=1)
        ...
        # Update layout
        fig.update_layout(height=1080, width=1920, title_text=f"Strategy vs Benchmark Performance (Training size: {train_size}) || params: {coin} | {tf} | {metrics_name} | {lookback_period} | {threshold} | {denoise_logic} | {isMomentum}",
...
def optimization(asset_list:list,metrics_name:str,
                 tf_list:list,lookback_period_list:list,
                 isMomentum_list:list,denoise_logic_list:list,threshold_list: Optional[List] = None,
                 denoise=denoise,metric_type:str='onchain',shift=None,unit_tc:float=0.0006,is_save_ts:bool=False):

    if threshold_list is None:
        if denoise_logic_list == ['z'] or denoise_logic_list == ['pct+z'] or denoise_logic_list == ['ln+z'] or denoise_logic_list == ['ln+pct+z'] or denoise_logic_list == ['vol+z'] or denoise_logic_list == ['power+z']:
    ...
    hyperparameter_combinations = list(
        itertools.product(asset_list,tf_list,isMomentum_list, lookback_period_list, threshold_list, denoise_logic_list))
